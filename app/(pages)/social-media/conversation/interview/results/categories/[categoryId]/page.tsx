...
import { MetadataType, InterviewSessionType, SubCategoryType } from '@/app/(pages)/social-media/conversation/ConversationModel/types';
...
const Page = () => {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      ...
      try {
        setIsLoading(true);
        const uri = `/social-media/conversation/interview/completed-by-categories/${categoryId}`;
        const response = await axios.get(appConfig.api.url(uri));
        const results: ExendedSubCategoryType[] = response.data.records || [];
            ...
            {subcategories.map((sub_category, index) => (
              <div key={index} className="mb-4">
                ...
                <p className="text-lg mb-2">
                  ...
                  <Link href={`/social-media/conversation/interview/results/categories/${categoryId}/sub-category/${sub_category.id}`}>
