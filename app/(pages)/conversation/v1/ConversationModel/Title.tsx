...
import CategoryType from '../../conversation/v1/categories/AutoModel/types';
import SubCategoryType from '../../conversation/v1/categories/[categoryId]/sub-categories/AutoModel/types';
...
const Title: React.FC<Props> = ({
    ...
    const fetchSubCategories = async (cat_id: string) => {
        try {
            const response = await axios.get<SubCategoryType[]>(appConfig.api.url(`/conversation/v1/categories/${cat_id}/sub-categories`));
    ...
    const archiveConversation = async () => {
        ...
        try {
            const resp = await axios.put(appConfig.api.url(`/conversation/v1/categories/${selectedCategory.id}/sub-categories/${selectedSubCategory.id}/archive`));
            if (resp.status === 200) {
                setMessages([]);
    ...
    useEffect(() => {
        const fetchConversation = async () => {
            ...
            try {
                const response = await axios.get<ResultsMetaDataType>(appConfig.api.url(`/conversation/v1/sub-categories/${selectedSubCategory.id}/conversation?c=1`));
