...
import CategoryType from '../../conversation/v2/categories/AutoModel/types';
import SubCategoryType from '../../conversation/v2/categories/[categoryId]/sub-categories/AutoModel/types';
...
const Title: React.FC<Props> = ({
    ...
    const fetchCategories = async () => {
        try {
            const response = await axios.get<CategoryType[]>(appConfig.api.url('/conversation/v2/categories'));
    ...
    const fetchSubCategories = async (cat_id: string) => {
        try {
            const response = await axios.get<SubCategoryType[]>(appConfig.api.url(`/conversation/v2/categories/sub-categories/?category_id=${cat_id}`));
    ...
    const archiveConversation = async () => {
        ...
        try {
            const resp = await axios.put(appConfig.api.url(`/conversation/v2/categories/sub-categories/archive?category_id=${selectedCategory.id}&sub_category_id=${selectedSubCategory.id}`));
            if (resp.status === 200) {
                setMessages([]);
    ...
    useEffect(() => {
        const fetchConversation = async () => {
            ...
            try {
                const response = await axios.get<ResultsMetaDataType>(appConfig.api.url(`/conversation/v2/sub-categories/conversation/?sub_category_id=${selectedSubCategory.id}`));
