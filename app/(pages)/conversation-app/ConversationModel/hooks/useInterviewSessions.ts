...
export const useInterviewSessions = (selectedCategory: CategoryType | undefined, selectedSubCategory: SubCategoryType | undefined) => {
    ...
    const fetchInterviewSessions = async () => {
        ...
        try {
            const response = await axios.get<InterviewSessionWithMetadataType>(appConfig.api.url(`/dashboard/interview/results/categories/${selectedCategory.id}/sub-category/${selectedSubCategory.id}?status_id=0&sess=1`));
