...
import Tooltip from "@/app/components/baseComponents/ToolTips/Tooltip";
...
const RecordMessage = ({ handleStop, messagesMetadata, isLoading, uploadFailed, conversationsContainer }: Props) => {
  ...
  const startTimer = () => {
    timerRef.current = setInterval(() => {
      ...
      // auto submit audio if maxDuration is reached
    ...
    <div className='h-full p-3 border-t text-center bg-gradient-to-r from-emerald-600 to-emerald-700'>
      {
        !messagesMetadata?.is_completed ?
                ...
                <div className="mt-2">
                  {
                    !isLoading ?
                      <>
                        <Tooltip text={`${recording ? 'Stop' : 'Start'} recording`}>
                          <button
                          ...
                          >
                            <RecordIcon
                              classText={`transition-all duration-500 ease-in-out ${status === 'recording' ? 'animate-pulse text-red-700' : ''}`}
                            />
                          </button>
                        </Tooltip>
                        <p className={`flex gap-1 justify-center mt-2 text-gray-800 ${status === 'recording' ? 'text-red-700' : ''}`}>
                          {
                            duration && <div className="text-emerald-300">{duration} secs</div>
                          }
                        </p>
                        <div>
                          <p>
                            {
                              ...
                              <div className="flex justify-center items-center gap-2">
                                <Tooltip text="Cancel">
                                  <div className="" style={{ cursor: 'pointer' }} onClick={abortRecording}>
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6 text-emerald-500">
                        ...
                        {!recording && uploadFailed ?
                          <Tooltip text="Unknown error occurred">
                            <div className="text-red-700 cursor-default flex justify-center items-center gap-1">
                              <span>Unknown error,</span>
                              <Tooltip text="Click to retry uploading">
                                <span onClick={() => blobUrl && handleStop(blobUrl)} className="cursor-pointer">
                      ...
                      <div className="flex justify-center items-center gap-2" role="status">
                        <Tooltip text="Uploading your recording...">
                          <div>
                  ...
                  <p>
                    <span className="text-emerald-300">Keep your message between 3 secs and {maxDuration} secs</span>
                  </p>
                </div>
              );
            }}
          ...
          <div className="flex items-center justify-center gap-2 mt-4 text-xl">
            ...
            <p className="text-white">You have completed the interview!</p>
            <span role="img" aria-label="confetti">🎉</span>
            <span role="img" aria-label="checkmark">✅</span>
          </div>
      }
