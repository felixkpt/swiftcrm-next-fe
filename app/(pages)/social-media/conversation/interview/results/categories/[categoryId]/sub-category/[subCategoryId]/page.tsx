...
import InterviewListCard from '@/app/(pages)/social-media/conversation/interview/session/components/InterviewListCard';
import { MetadataType, ResultsType } from '@/app/(pages)/social-media/conversation/ConversationModel/types';
...
const Page = () => {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      ...
      try {
        setIsLoading(true);
        const uri = `/social-media/conversation/interview/results/categories/${categoryId}/sub-category/${subCategoryId}`;
        const response = await axios.get(appConfig.api.url(uri));
        const data = response.data;

        // Prepare chart data
        const chartLabels = data.records.map((interview: any) => interview.question);
        const chartScores = data.records.map((interview: any) => interview.score);
