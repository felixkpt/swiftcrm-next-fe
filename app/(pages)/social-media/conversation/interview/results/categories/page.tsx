...
import { CategoryType, InterviewSessionType, MetadataType } from '@/app/(pages)/social-media/conversation/ConversationModel/types';
...
const Page = () => {
  ...
  useEffect(() => {
    ...
    const fetchResults = async () => {
      setIsLoading(true);
      try {
        const response = await axios.get(appConfig.api.url('/social-media/conversation/interview/completed-by-categories'));
            ...
            {categories.map((category, index) => (
              <div key={index} className="mb-4">
                ...
                <p className="text-lg mb-2">
                  <Link href={`/social-media/conversation/interview/results/categories/${category.id}`}>
