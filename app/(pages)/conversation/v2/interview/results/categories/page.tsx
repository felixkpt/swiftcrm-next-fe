...
import { CategoryType, InterviewSessionType, MetadataType } from '@/app/(pages)/conversation/v2/ConversationModel/types';
...
const Page = () => {
  ...
  useEffect(() => {
    ...
    const fetchResults = async () => {
      setIsLoading(true);
      try {
        const response = await axios.get(appConfig.api.url('/conversation/v2/interview/completed-by-categories'));
            ...
            {categories.map((category, index) => (
              <div key={index} className="mb-4">
                ...
                <p className="text-lg mb-2">
                  <Link href={`/conversation/v2/interview/results/categories/${category.id}`}>
