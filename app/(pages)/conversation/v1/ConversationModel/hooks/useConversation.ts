...
import CategoryType from '@/app/(pages)/conversation/v1/categories/AutoModel/types';
...
export const useConversation = (
    ...
    useEffect(() => {
        const fetchConversation = async () => {
            ...
            try {
                const uri = `/conversation/v1/sub-categories/${selectedSubCategory.id}/conversation?mode=${mode}&interview_id=${currentSessionId}`;
    ...
    const getInterviewSession = async () => {
        ...
        const uri = `/conversation/v1/interview/${selectedSubCategory.id}/progress?interview_id=${currentSessionId}`;
        try {
