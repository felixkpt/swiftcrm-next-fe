...
import SubCategoryType from '@/app/(pages)/conversation/v2/categories/[categoryId]/sub-categories/AutoModel/types';
...
export const useInterviewSessions = (selectedCategory: CategoryType | undefined, selectedSubCategory: SubCategoryType | undefined) => {
    ...
    const fetchInterviewSessions = async () => {
        ...
        try {
            const response = await axios.get<InterviewSessionWithMetadataType>(appConfig.api.url(`/conversation/v2/interview/results/categories/${selectedCategory.id}/sub-category/${selectedSubCategory.id}?status_id=0&sess=1`));
