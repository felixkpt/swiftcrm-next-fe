...
const TrainingSubNavMenu: React.FC<SubNavMenuProps> = ({
    ...
    const archiveConversation = async () => {
        ...
        try {
            const resp = await axios.put(appConfig.api.url(`/conversation/v1/categories/${selectedCategory.id}/sub-categories/${selectedSubCategory.id}/archive?mode=${mode}`));
            if (resp.status === 200) {
                setMessages([]);
    ...
    useEffect(() => {
        const fetchConversation = async () => {
            ...
            try {
                const uri = `/conversation/v1/sub-categories/${selectedSubCategory.id}/conversation?mode=${mode}`
